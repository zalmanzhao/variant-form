    ...
    <div :key="widget.id" class="sub-form-container"
      ...
      <div v-for="(subFormRowId, sfrIdx) in rowIdData" class="sub-form-row" :key="subFormRowId">
        ...
        <div class="grid-sub-form-data-row">
          <component v-for="(subWidget, swIdx) in widget.widgetList" :key="widgetSchemaData[sfrIdx][swIdx].id" :is="getComponentByContainer(subWidget)" :widget="widgetSchemaData[sfrIdx][swIdx]"
                      :parent-list="widget.widgetList"
                      :index-of-parent-list="swIdx" :parent-widget="widget"
                      :sub-form-row-id="subFormRowId" :sub-form-row-index="sfrIdx" :sub-form-col-index="swIdx" :preview-state="previewState">
            <!-- 子表单暂不支持插槽！！！ -->
          </component>
        </div>
        <div v-if="!leftActionColumn" class="sub-form-action-column hide-label">
  ...
  export default {
    ...
    methods: {
      ...
      handleSubFormFirstRowAdd() {
        if ("grid-sub-form" === this.widget.type && this.widget.options.showBlankRow && 1 === this.rowIdData.length) {
