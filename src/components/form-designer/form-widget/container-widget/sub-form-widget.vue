        ...
        <div class="sub-form-table">
          <draggable :list="widget.widgetList" class="draggable-div" v-bind="{group:'dragGroup', ghostClass: 'ghost',animation: 200}"
            ...
            <transition-group name="fade" tag="div" class="form-widget-list">
                <div v-for="(subWidget, swIdx) in widget.widgetList" :key="subWidget.id"  class="sub-form-table-column" :style="{width: subWidget.options.columnWidth}">
                  <component :is="subWidget.type + '-widget'" :field="subWidget" :designer="designer" :key="subWidget.id" :parent-list="widget.widgetList"
                          :index-of-parent-list="swIdx" :parent-widget="widget" :design-state="true" :sub-form-item-flag="true"></component>
                </div>
            </transition-group>
          </draggable>
        </div>
